...
function groupByTopicPartition (payloads) {
  ...
  // group by topic and then partition
  return payloads.reduce((out, payload) => {
    out[payload.topic] = out[payload.topic] || {}

    if (out[payload.topic][payload.partition]) {
      // merge messages
      let messages = out[payload.topic][payload.partition].messages
      messages.push.apply(messages, payload.messages)

    } else {
      out[payload.topic][payload.partition] = payload
    }
