...
function groupByTopicPartition (payloads) {
  ...
  // concat messages into 1 payload for payloads with the same topic and partition
  payloads.forEach(payload => {
    let key = payload.topic + ':' + payload.partition

    if (!topicPartitionMap[key]) {
      topicPartitionMap[key] = payload
      out.push(payload)
    } else {
      // merge messages
      let messages = topicPartitionMap[key].messages
